...
aps_neg = aps.loc[aps['class']==-0.9921875, :] 
np.random.seed(12)
idx = np.random.randint(0, aps_neg.shape[0], 1375)  # we just randomly select parts of negative samples 
aps_neg = aps_neg.iloc[idx, :]

dat = pd.concat([aps_neg, aps_pos], axis=0)

X = dat.iloc[:, 1:].to_numpy()
label = dat.iloc[:, 0].to_numpy()
...
def run(seed): 
    ...
    '''
    We find that in this case, not using spectral embedding will perform better. 
    '''
    embeddings = X_shuffle  # spectral_embeddings(X_shuffle, graph, mode, n_neighbors, embedding_dim)
...
#     acp_RI, acp_MI, acp_HS, acp_CS  = metrics(label, logits.argmax(dim=1))
    ...
    y_rgcl = RGCL(X_shuffle, K, seed=seed)
    results[5, :] = metrics(label_shuffle, y_rgcl)
    
    y_srgcl = RGCL(X_shuffle, K, sus_exp=True, seed=seed)
...
if __name__ == '__main__':   
    np.random.seed(8115)
    rds = np.random.choice(range(1, 18), size=5, replace=False)
