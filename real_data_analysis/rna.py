...
#     acp_RI, acp_MI, acp_HS, acp_CS  = metrics(label, logits.argmax(dim=1))
    ...
    sc = cluster.SpectralClustering(n_clusters=K, n_components=K, affinity='nearest_neighbors', n_neighbors=n_neighbors, random_state=seed).fit(X_shuffle)  # rbf 依然解决不了circle问题
    y_pred = sc.labels_
